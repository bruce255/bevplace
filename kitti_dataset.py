    ...
    for q_idx, pred in enumerate(predictions):
        ...
        if len(positives)>0:
            ...
            if match_results_save_path is not None:
                ...
                # 優化：直接讀取不調用 dataset[idx] 避免多層 Tensor 封裝浪費記憶體
                ...
                # 🔥【防OOM優化】儲存圖片後，立刻手動銷毀這些龐大影像
                del query_im, db_im, im, out_im, im_warp
                if q_idx % 20 == 0:
                    gc.collect()
...
class TrainingDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        query = cv2.warpAffine(query, mat, (query.shape[1], query.shape[0]))
        ...
        positive = cv2.warpAffine(positive, mat, (positive.shape[1], positive.shape[0]))
        ...
        for neg_i in neg_idx:
            ...
            negative = cv2.warpAffine(negative, mat, (negative.shape[1], negative.shape[0])) 
